#!/bin/env python
"""
Susanoo transmits information about PC state to the 5x5x3
keyboard. This makes that the rgb lights on 5x5x3 can accurately
...
SOCKET_NAME = f"/run/user/{os.getuid()}/5x5x3/socket"
FIFO_NAME = f"/run/user/{os.getuid()}/5x5x3/fifo"
...
class PulseListener:
    ...
    async def update_sound_output(self, pulse):
        ...
        await self.mux.terminal_line_received(
            f"lightgroup volume {volume:04x}\nlightgroup mute {mute:01x}\n".encode()
        )

    async def update_sound_input(self, pulse):
        sources = await pulse.source_list()
        micmute = all(s.mute for s in sources)
        await self.mux.terminal_line_received(f"lightgroup micmute {micmute:01x}\n".encode())
...
def find_5x5x3():
    """Find 5x5x3 keyboard device name"""
    context = pyudev.Context()
    return next(
        device.properties["DEVNAME"]
        for device in context.list_devices(subsystem="tty")
        if device.properties.get("ID_MODEL") == "5x5x3"
    )
